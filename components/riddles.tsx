...
export default function Riddles() {
    ...
    useEffect(() => {
    //     const allRiddles = [r.default.riddles1,r.default.riddles2,r.default.riddles3,r.default.riddles4,r.default.riddles5,r.default.riddles6]
    //     allRiddles.map((item, index)=>{
    //     setDoc(doc(db, "riddles", item.name + "(" + index + ")"), item);
    // })
    ...
    return (
        <View style={{ flex: 1, margin: 10 }}>
            ...
            <View style={{position:'absolute', bottom:100, flex:1, flexDirection:'row', backgroundColor:'cadetblue',width:'76%', left:'12%',justifyContent:'space-between', borderRadius:20 }}>
            ...
            :<ScrollView>
            <Text>{auth.currentUser?.displayName && auth.currentUser?.displayName}</Text>
            <Text style={{ textAlign: 'center', fontSize: 30, textDecorationLine: 'underline' }}>חידות </Text>
            <TouchableOpacity onPress={getAllRiddles} style={{ width: 50 }}><Feather name="refresh-ccw" size={40} color="cadetBlue" /></TouchableOpacity>
            <Text>{`\n`}</Text>
           {riddles.map((item,index)=><View key={item.name+index}><TouchableOpacity  style={{margin: 5, marginHorizontal:30 , borderWidth:3, borderColor:"cadetblue", borderRadius:10}} onPress={() => chooseRiddle(index)}><Text style={{ textAlign: 'center', fontSize: 25 }}>{item.name}</Text>
                    <Image
